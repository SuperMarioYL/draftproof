...
录制脚本：[docs/demo.tape](docs/demo.tape)（vhs，可用 [.github/workflows/demo.yml](.github/workflows/demo.yml) 重新渲染）。动图是同一流程的**另一次独立录制**——其中的临时目录、指纹与时间戳属于那次录制会话，并非下方文字输出所属的同一次运行。
...
演示脚本随即把 seq 2 的 `content_hash` 改掉 8 个十六进制字符（独立复现可用 `python3 examples/tamper-receipt.py`），再验一次：
